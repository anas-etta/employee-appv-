...
public class BatchConfig {
    ...
    private final BatchErrorLogService errorLogService; // ✅ Ajout du service pour gérer les erreurs

    // 📂 🔥 Nouveau chemin absolu
    private static final String DIRECTORY_PATH = "C:/Users/anase/Downloads/batch (3)/batch (3)/batch/src/dossier_ndjson";
    ...
    public Job importEmployeeJob(JobRepository jobRepository, Step processFilesStep, FileMoveListener fileMoveListener) {
        return new JobBuilder("importEmployeeJob", jobRepository)
                ...
                .listener(fileMoveListener) // ✅ Ajout du listener
    ...
    public Step processFilesStep(JobRepository jobRepository,
        ...
        return new StepBuilder("processFilesStep", jobRepository)
                ...
                .reader(reader()) // 🔥 Utilise le lecteur mis à jour avec gestion des erreurs
                .processor(processor())
                .writer(compositeWriter())
                .allowStartIfComplete(true) // ✅ Permet de relancer même si déjà complété
                .build();
    }

    @Bean
    public NDJsonItemReader reader() {
        // 🔥 Maintenant, on passe aussi errorLogService
